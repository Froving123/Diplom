...
export const ConfirmedOrder = () => {
  ...
  const fetchAcceptOrders = async () => {
    try {
      const response = await fetch("/api/manord/acceptOrdersGet", {
        method: "GET",
        headers: {
  ...
  const readyOrder = async (orderId) => {
    try {
      const response = await fetch("/api/manord/readyOrder", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
              ...
              <li key={order.orderId} className={Styles.li_new_order}>
                ...
                <div className={Styles.buttons}>
                  <button
                    ...
                    onClick={() => readyOrder(order.orderId)}
                  >
                    <p className={Styles.accept_text}>Готов</p>
                  </button>
                </div>
              </li>
            );
        ...
        <ul className={Styles.ul_new_order}>
          <li className={Styles.li_new_order}>
            <p className={Styles.new_order_empty}>
              Сейчас нет готовищихся заказов
            </p>
          </li>
        </ul>
      )}
